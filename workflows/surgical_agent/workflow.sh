...
find_video_streaming_containers() {
    # Find containers with specific holohub image running the exact live_video_streaming_server command
    # This is more robust than simple string matching to avoid conflicts with other containers
    docker ps --format "{{.ID}}\t{{.Image}}\t{{.Command}}" | \
        awk '$2 ~ /^holohub:/ && $3 ~ /\.\/holohub run live_video_streaming_server/ {print $1}' || true
}
...
clone_surgical_framework() {
    ...
    if [ -d "VLM-Surgical-Agent-Framework" ]; then
        ...
        git pull origin apatole/add_live_streaming_support
    else
        git clone -b apatole/add_live_streaming_support https://github.com/apatole/VLM-Surgical-Agent-Framework.git
        cd VLM-Surgical-Agent-Framework
    fi
