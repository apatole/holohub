...
class WebAppThread(Thread):
    ...
    def run(self):
        ...
        logging.info(f"Starting WebRTC streaming server (API only) at {self._host}:{self._port} ({cors_status})")
        logging.info("Available endpoints:")
        logging.info(f"  POST http://{self._host}:{self._port}/offer - WebRTC signaling")
        logging.info(f"  GET  http://{self._host}:{self._port}/iceServers - ICE server configuration")
